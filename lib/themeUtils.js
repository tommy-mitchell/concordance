...
function withDefaults(theme) {
	return freezeTheme(defaults(theme, defaultTheme));
}
...
function normalizePlugins(plugins) {
	...
	const theme = withDefaults(Object.assign({}, ...themes));
...
export function normalize(options) {
	...
	if (!options.theme) {
		return normalizedPlugins?.theme ?? defaultTheme;
	}
	...
	if (normalizedPlugins) {
		...
		const theme = withDefaults(options.theme);
		entry.withPlugins.set(normalizedPlugins.ref, theme);
		return theme;
	}

	entry.theme ??= withDefaults(options.theme);
...
export function applyModifiers(descriptor, theme) {
	...
	return [...modifiers.get(descriptor)].reduce((previous, modifier) => {
		...
		const modifiedTheme = structuredClone(previous);
